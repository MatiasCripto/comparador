// scrape_shopify.mjs — Scraper de productos Shopify
// Endpoint: /collections/all/products.json?limit=250&page=1
// Fallback: /products.json, HTML scraping
// Uso: node scrape_shopify.mjs
...
const API_DELAY_MS = 2000; // Shopify is aggressive with rate limiting
...
async function fetchJson(url, timeoutMs = 15000) {
  ...
  try {
    ...
    if (!res.ok) return { error: 'HTTP ' + res.status, status: res.status, headers: res.headers };
...
function extractFromShopifyApi(json, baseUrl) {
  ...
  return products.map(p => {
    ...
    const price = minPrice === Infinity ? parsePrice(variants[0]?.price || 0) : minPrice;
    const handle = p.handle || '';

    return {
      name: (p.title || '').trim(),
      price,
      regular_price: comparePrice > price ? comparePrice : price,
      currency: 'ARS',
      ...
      stock_status: variants.some(v => v.available !== false) ? 'instock' : 'outofstock',
    };
  }).filter(p => p.name && p.price > 0);
}

// ------ SHOPIFY HTML EXTRACTOR ------

function extractShopifyFromHtml(html, baseUrl) {
  const products = [];
  const seen = new Set();
  ...
  // Método 2: Shopify HTML structure — <div class="grid-product">
  const blocks = [];
  const gridRegex = /<div[^>]*class=["'][^"']*(?:grid-product|product-item|product-card|product-index)[^"']*["'][^>]*>[\s\S]*?<\/div>\s*<\/div>/gi;
  let match;
  while ((match = gridRegex.exec(html)) !== null) {
    if (match[0].length < 3000) blocks.push(match[0]);
  }

  for (const block of blocks) {
    const nameMatch = block.match(/<div[^>]*class=["'][^"']*product-title[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)
      || block.match(/<a[^>]*>([^<]{4,120})<\/a>/i);
    if (!nameMatch) continue;
    const name = nameMatch[1].replace(/<[^>]*>/g, '').trim();
    if (seen.has(name) || name.length < 3 || name.length > 120) continue;

    const priceMatch = block.match(/<span[^>]*class=["'][^"']*(?:price|product-price)[^"']*["'][^>]*>\s*\$?([\d.,]+)\s*<\/span>/i)
      || block.match(/<span[^>]*class=["'][^"']*money["'][^>]*>\s*\$?([\d.,]+)\s*<\/span>/i)
...
async function upsertProducts(storeId, products, category) {
  ...
  for (const p of products) {
    ...
    if (exist) {
      ...
      priceRows.push({
        product_id: exist.id, store_id: storeId,
        price: p.price,
      ...
      if (res.ok) {
        ...
        if (Array.isArray(created)) {
          ...
          for (let j = 0; j < created.length; j++) {
            const globalIdx = i + j;
            const p = globalIdx < products.length ? products[globalIdx] : null;
            if (p) {
              priceRows.push({
                product_id: created[j].id, store_id: storeId,
                price: p.price,
                price_original: p.regular_price || p.price,
                currency: 'ARS',
                is_offer: (p.regular_price || 0) > 0 && p.price < (p.regular_price || 0),
...
async function main() {
  console.log('--- Scraper Shopify ---\n');

  const storesRes = await fetch(
    SUPABASE_URL + '/rest/v1/stores?select=id,name,url,category,scraping_config&scraping_config->>platform_detected=eq.Shopify&limit=50',
    { headers: AUTH }
  );
  let stores = await storesRes.json();
  if (!Array.isArray(stores) || stores.length === 0) {
    console.log('No se encontraron tiendas Shopify');
    return;
  }

  console.log(`Tiendas Shopify: ${stores.length}\n`);
  ...
  for (let si = 0; si < stores.length; si++) {
    ...
    // FASE 1: Shopify /collections/all/products.json (works with rate limiting)
    if (products.length === 0) {
      for (let page = 1; page <= 4; page++) {
        await sleep(API_DELAY_MS); // Respect rate limits
        const apiUrl = `${base}/collections/all/products.json?limit=250&page=${page}`;
        const result = await fetchJson(apiUrl);
        if (result.data && !result.error) {
          const batch = extractFromShopifyApi(result.data, base);
          const newOnes = batch.filter(p => !products.some(e => e.url === p.url));
          products.push(...newOnes);
          console.log(`   API page ${page}: ${batch.length} productos`);
      ...
      if (products.length > 0) method = `api_collections_all (${products.length})`;
    }

    // FASE 2: Shopify /products.json
    if (products.length === 0) {
      for (let page = 1; page <= 4; page++) {
      ...
      if (products.length > 0) method = `api_products_json (${products.length})`;
    }

    // FASE 3: HTML scraping
    if (products.length === 0) {
      const htmlPaths = ['', '/collections/all', '/products'];
      for (const path of htmlPaths) {
        if (products.length > 30) break;
        const html = await fetchHtml(base + path);
        if (html && html.length > 1000) {
          const extracted = extractShopifyFromHtml(html, base);
